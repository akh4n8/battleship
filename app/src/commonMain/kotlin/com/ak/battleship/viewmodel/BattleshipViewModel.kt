...
import com.ak.battleship.utils.createSettingsManager
...
class BattleshipViewModel(private val dao: GameRepository, private val context: Any?) : ViewModel() {
    // --- DEBUGGING LAYERS ---
    ...
    private val settingsManager = createSettingsManager(context)

    var isTutorialDialogVisible by mutableStateOf(
        !settingsManager.getBoolean("has_seen_tutorial_v1", false)
    ); private set
    ...
    private val _currentWidgetTheme = MutableStateFlow(
        try {
            WidgetTheme.valueOf(settingsManager.getString("widget_theme", WidgetTheme.STANDARD_GAUGE.name) ?: WidgetTheme.STANDARD_GAUGE.name)
        } catch (e: Exception) {
    ...
    fun cycleWidgetTheme() {
        ...
        settingsManager.setString("widget_theme", nextTheme.name)
    }
    ...
    fun loadGame(gameId: Int, context: Any? = null) {
        viewModelScope.launch {
    ...
    private suspend fun loadGameSuspended(gameId: Int, context: Any? = null) {
        clearTemporalState()
                ...
                withContext(Dispatchers.Default) {
                    ...
                    if (isMoriarty) {
                        ...
                        val mlPrior = com.ak.battleship.ai.MoriartyBot.getPsychologicalPrior(
                            context = context ?: this@BattleshipViewModel.context,
    ...
    fun confirmShipPlacement(context: Any? = null) {
    ...
    fun recordMove(x: Int, y: Int, result: String, isSunk: Boolean, context: Any? = null, autoSwitchTurn: Boolean = true) {
        viewModelScope.launch { recordMoveSuspended(x, y, result, isSunk, context, autoSwitchTurn) }
    }

    private suspend fun recordMoveSuspended(x: Int, y: Int, result: String, isSunk: Boolean, context: Any?, autoSwitchTurn: Boolean) {
        val gameId = currentGameId ?: return
        ...
        if (offenseHits.size >= 17 || defenseHits >= 17) {
            ...
            if (isBotGame) {
                ...
                com.ak.battleship.PlatformServices.showToast(context, if (isWin) "Victory! Verify ships now..." else "Defeat! Verify ships now...")
    ...
    fun saveOpponentShipsAndFinish(context: Any? = null) {
        val gameId = currentGameId ?: return
        viewModelScope.launch {
    ...
    fun fireAtBot(x: Int, y: Int, context: Any? = null) {
        ...
        if (alreadyShot) {
            com.ak.battleship.PlatformServices.showToast(context, "Target already engaged!")
            return
        }
    ...
    fun firePassAndPlay(x: Int, y: Int, context: Any? = null) {
        if (isActionLocked || currentGame?.result != null || !isOffenseMode) return
    ...
    private fun executeBotTurnLoop(context: Any?) {
        botTurnJob?.cancel()
        botTurnJob = viewModelScope.launch {
            if (currentGame?.result != null) return@launch

            val startTime = com.ak.battleship.PlatformServices.getCurrentTimeMillis()
            ...
            val elapsed = com.ak.battleship.PlatformServices.getCurrentTimeMillis() - startTime
    ...
    fun editOpponentShips(gameId: Int, context: Any? = null) {
        viewModelScope.launch {
            loadGameSuspended(gameId, context)
    ...
    fun processCsvImport(context: Any?, csvText: String) {
        viewModelScope.launch {
            val lines = csvText.lines().map { it.trim() }.filter { it.isNotEmpty() }
            if (lines.isEmpty() || !lines[0].contains("GameID")) {
                com.ak.battleship.PlatformServices.showToast(context, "Invalid CSV data")
                return@launch
            }
            ...
            com.ak.battleship.PlatformServices.showToast(context, "Imported $importedCount new games")
        }
    }

    fun handleSmartImport(context: Any?, uri: Any?) {
        viewModelScope.launch {
            try {
                val fileContent = com.ak.battleship.PlatformServices.readUriText(context, uri) ?: return@launch

                if (fileContent.trim().startsWith("{") || fileContent.trim().startsWith("[")) {
                    println("Successfully imported AI weights! Size: ${fileContent.length} chars")
                    com.ak.battleship.PlatformServices.showToast(context, "AI Brain Imported!")
                } else if (fileContent.contains("GameID")) {
                    processCsvImport(context, fileContent)
                } else {
                    com.ak.battleship.PlatformServices.showToast(context, "Unrecognized file format")
                }
            } catch (e: Exception) {
                e.printStackTrace()
                com.ak.battleship.PlatformServices.showToast(context, "Failed to read file")
            }
        }
    }
