...
private object AdlerBot {
    ...
    fun getBestMove(moves: List<Move>, offensivePrior: Array<FloatArray>?, playerName: String, gameId: Int): BotDecision {
        ...
        if (analysis.activeHits.isNotEmpty()) {
            ...
            if (target != null) {
                val logMessage = buildString {
                    ...
                    append("Bias Skew: [Max: ${maxBias.formatDecimal(2)}x, Min: ${minBias.formatDecimal(2)}x]\n")
        ...
        val logMessage = buildString {
            ...
            append("Data Array: $matrixTypeTag\n")
            append("Bias Skew: [Max: ${maxBias.formatDecimal(2)}x, Min: ${minBias.formatDecimal(2)}x]\n")
            append("Peak Node Weight: ${peakBayesianHeat.formatDecimal(1)}")
        }
...
private object MycroftBot {
    ...
    fun getBestMove(moves: List<Move>, gameId: Int, moriartyPrior: Array<FloatArray>? = null, isMoriarty: Boolean = false): BotDecision {
        ...
        for (step in 0 until 100_000) {
            if (step % 1000 == 0 && com.ak.battleship.PlatformServices.getCurrentTimeMillis() - startTime > maxTimeMs) break
                        ...
                        for (i in 0..4) {
                            ...
                            if ((sl and allHitsLow) == sl && (sh and allHitsHigh) == sh) {
                                ...
                                while (tempSl != 0L) {
                                    ...
                                    val b = lsb.countTrailingZeroBits()
                                    val t = pegTimes[b]
                                    if (t > maxTime) {
            ...
            if (step >= burnInPeriod) {
                ...
                for (i in 0..4) {
                    ...
                    while (sl != 0L) {
                        val lsb = sl and -sl
                        rawHeatmap[lsb.countTrailingZeroBits()]++
                        sl = sl xor lsb
                    }
                    var sh = stateHigh[i]
                    while (sh != 0L) {
                        val lsb = sh and -sh
                        rawHeatmap[lsb.countTrailingZeroBits() + 64]++
                        sh = sh xor lsb
                    }
                }
            }
...
object MoriartyBot {
                ...
                if (move.x in 0..9 && move.y in 0..9) {
                    ...
                    if (isHit) {
                        ...
                        for ((hx, hy) in hitQueue) {
                            if (kotlin.math.abs(hx - move.x) <= 1 && kotlin.math.abs(hy - move.y) <= 1) {
    ...
    fun getPsychologicalPrior(
        ...
        val numChannels = if (isOffense) 5 else 7
        val logits = TensorFlowBridge.runMoriartyInference(context, isOffense, playerName, flatData, steps, numChannels) ?: return null
        val probabilities = FloatArray(100) { i -> (1.0 / (1.0 + kotlin.math.exp(-logits[i].toDouble()))).toFloat() }
    ...
    fun generatePhantomFleet(defensivePrior: Array<FloatArray>?): List<Ship> {
        ...
        for ((size, name) in shipSpecs) {
            ...
            if (validPlacements.isNotEmpty()) {
                val weights = heatScores.map { kotlin.math.exp(-it.toDouble()) }
                val totalWeight = weights.sum()
                var randomVal = kotlin.random.Random.nextDouble() * totalWeight
...
private fun Float.formatDecimal(decimals: Int = 2): String {
